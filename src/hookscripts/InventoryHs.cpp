...
#include "..\inventory.h"
#include "..\sfall\Modules\PartyControl.h"
#include "Common.h"
...
{
/*
static void RemoveInvenObjHook_Script(fallout::Object* source, fallout::Object* item, long count, long rmType) {
	BeginHook();
	argCount = 5;
...
void RemoveInvenObjHook_Invoke(fallout::Object* source, fallout::Object* item, long count, long rmType) {
...
static __declspec(naked) void MoveCostHook() {
	__asm {
		...
		mov  args[0], eax;
		mov  args[4], edx;
		call fallout::critterGetMovementPointCostAdjustedForCrippledLegs;
		mov  args[8], eax;
		push ebx;
		push ecx;
		mov  ebx, eax;
...
static int __fastcall SwitchHandHook_Script(fallout::Object* item, fallout::Object* itemReplaced, DWORD addr) {
    if (itemReplaced && fallout::itemGetType(itemReplaced) == fallout::ITEM_TYPE_WEAPON && fallout::itemGetType(item) == fallout::ITEM_TYPE_AMMO) {
	...
	int result = sfall::PartyControl::SwitchHandHook(item);
	if (result != -1) {
		cRetTmp = 0;
...
static __declspec(naked) void DropIntoContainerHandSlotHack() {
	...
	__asm {
		call fallout::_drop_into_container;
...
static __declspec(naked) void DropAmmoIntoWeaponHook() {
	...
	__asm {
		...
		jne  donothing;
		popadc;
		jmp  fallout::weaponCanBeReloadedWith;
donothing:
...
static long __fastcall InvenWieldHook_Script(fallout::Object* critter, fallout::Object* item, long slot, long isWield, long isRemove) {
	if (!isWield) {
		// for the critter, the right slot is always the active slot
		if (slot == fallout::INVEN_TYPE_LEFT_HAND && critter != fallout::gDude) return 1;
		// check the current active slot for the player
		if (slot != fallout::INVEN_TYPE_WORN && critter == fallout::gDude) {
			long _slot = (slot != fallout::INVEN_TYPE_LEFT_HAND);
			if (_slot != fallout::gInterfaceCurrentHand) return 1; // item in non-active slot
		}
	}
	BeginHook();
	argCount = 5;
...
static __declspec(naked) void InvenWieldFuncHook() {
	...
	// right hand slot?
	if (args[2] != fo::INVEN_TYPE_RIGHT_HAND && fallout::itemGetType((fallout::Object*)args[1]) != fallout::ITEM_TYPE_ARMOR) {
		args[2] = fallout::INVEN_TYPE_LEFT_HAND;
	}
...
__declspec(noinline) fallout::Object* GetItemPtrSlot(fallout::Object* critter, fallout::InvenType slot)
{
    fallout::Object* itemPtr = nullptr;
    switch (slot) {
    case fallout::INVEN_TYPE_LEFT_HAND:
        itemPtr = critterGetItem1(critter);
        break;
    case fallout::INVEN_TYPE_RIGHT_HAND:
        itemPtr = critterGetItem2(critter);
        break;
    case fallout::INVEN_TYPE_WORN:
        itemPtr = critterGetArmor(critter);
        break;
    }
    return itemPtr;
}
...
static __declspec(naked) void InvenUnwieldFuncHook() {
	...
	// get item
	args[1] = (DWORD)GetItemPtrSlot((fallout::Object*)args[0], (fallout::InvenType)args[2]);
...
static __declspec(naked) void CorrectFidForRemovedItemHook() {
	...
	__asm {
		popadc;
		jmp  fallout::_correctFidForRemovedItem;
	}
}
...
runHook:
		...
		test edx, edx;
		jz   left;
		call fallout::critterGetItem2;
		jmp  skip;
left:
		call fallout::critterGetItem1;
skip:
		test eax, eax;
		jz   noWeapon;
		push 1; // remove event
...
static void AdjustFidHook(DWORD vanillaFid) {
	...
	args[0] = vanillaFid;
	args[1] = fallout::gInventoryWindowDudeFid; // modified FID by sfall code
	RunHookScript(HOOK_ADJUSTFID);

	if (cRet > 0) {
		fallout::gInventoryWindowDudeFid = rets[0];
	}
	EndHook();
}
