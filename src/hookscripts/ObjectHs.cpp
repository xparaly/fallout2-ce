...
#include "..\sfall\Modules\CritterPoison.h"
#include "Common.h"
#include "ObjectHs.h"
...
static long UseObjOnHook_Script(fallout::Object* source, fallout::Object* item, fallout::Object* target) {
	BeginHook();
	argCount = 3;
...
static DWORD __fastcall AdjustPoison_Script(DWORD critter, long amount, DWORD addr) {
	...
	args[0] = critter;
	args[1] = amount;
	args[2] = (checkPoison) ? sfall::CritterPoison::adjustPoisonHP_Default : 0;

	RunHookScript(HOOK_ADJUSTPOISON);

	if (cRet > 0) amount = rets[0];
	if (cRet > 1 && checkPoison && (long)rets[1] < 0) sfall::CritterPoison::adjustPoisonHP = rets[1];
...
static __declspec(naked) void critter_adjust_poison_hack() {
	__asm {
		...
		jmp  sfall::critter_adjust_poison_hack_fix; // in CritterPoison
	}
}
