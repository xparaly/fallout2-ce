...
static DWORD __fastcall BarterPriceHook_Script(fallout::Object* source, fallout::Object* target, DWORD callAddr) {
    bool barterIsParty = (fallout::gGameDialogSpeakerIsPartyMember != 0);
    long computeCost = fallout::_barter_compute_value(source, target);
	...
	fallout::Object* bTable = fallout::_btable;
	args[3] = (DWORD)bTable;
	args[4] = itemGetTotalCaps(bTable);
	args[5] = objectGetCost(bTable);

	fallout::Object* pTable = fallout::_ptable;
	args[6] = (DWORD)pTable;
	...
	if (barterIsParty) {
		...
		args[7] = pcCost = objectGetCost(pTable);
	}
...
void BarterPriceHook_GetLastCosts(long& outPcTableCost, long& outNpcTableCost)
{
    if (!HookScripts::HookHasScript(HOOK_BARTERPRICE)) {
        outPcTableCost = objectGetCost(fallout::_ptable);
        outNpcTableCost = fallout::_barter_compute_value(fallout::gDude, fallout::_target_stack[0]);
        return;
    }
...
static fallout::Object* sourceSkillOn = nullptr;
void SourceUseSkillOnInit() { sourceSkillOn = fallout::gDude; }
...
static void __fastcall UseSkillOnHook_Script(DWORD source, DWORD target, DWORD skillId)
{
    ...
    sourceSkillOn = fallout::gDude;
    resultSkillOn = 0;
    bakupCombatState = -1;

    RunHookScript(HOOK_USESKILLON);

    if (skillId != fallout::SKILL_STEAL && cRet > 0) { // not work for steal skill
        if (rets[0] != 0) {
            resultSkillOn = (rets[0] == -1) ? -1 : 1;
            if (resultSkillOn == 1) {
                                       0x42BC87,
                                       0x42BC87,
                sourceSkillOn = (fallout::Object*)rets[0];
            }
        }
        if (resultSkillOn != -1 && cRet > 1 && rets[1] == 1) {
            bakupCombatState = fallout::gCombatState;
            fallout::gCombatState = 0;
        }
    }
    EndHook();
}
...
defaultHandler:
		HookEnd;
		jmp  fallout::skillsPerformStealing;
    }
}
...
skip:
		HookEnd;
		jmp  fallout::queueAddEvent;
    }
}

static __forceinline long PerceptionRangeHook_Script(fallout::Object* watcher, fallout::Object* target, int type, long result)
{
    BeginHook();
    argCount = 4;
...
static long __fastcall PerceptionRange_Hook(fallout::Object* watcher, fallout::Object* target, int type)
{
    return PerceptionRangeHook_Script(watcher, target, type, isWithinPerception(watcher, target));
}
...
static void CarTravelHook_Script()
{
    ...
    // }
    //// consume fuel
    // fo::var::carGasAmount = std::max(0, originalGas - consumption);
...
{
    int old = *(fallout::gGameGlobalVars + number);

    if (sfall::IsGameLoaded() && HookScripts::HookHasScript(HOOK_SETGLOBALVAR)) { // IsGameLoaded - don't execute hook until loading sfall scripts
        BeginHook();
        argCount = 2;
        args[0] = number;
        args[1] = value;
    ...
    if (number == fallout::GVAR_PLAYER_REPUTATION && sfall::displayKarmaChanges) {
        int diff = value - old;
        if (diff != 0) sfall::Karma::DisplayKarma(diff);
    }
    return value;
}
...
static int __fastcall ExplosiveTimerHook_Script(DWORD& type, DWORD item, DWORD time)
{
    ...
    args[0] = time;
    args[1] = item;
    args[2] = (type == 11) ? fallout::ROLL_FAILURE : fallout::ROLL_SUCCESS;

    RunHookScript(HOOK_EXPLOSIVETIMER);

    time = -1;
    if (cRet > 0 && (long)rets[0] >= 0) {
        time = std::min((int)rets[0], 18000); // max 30 minutes
        if (cRet > 1) {
            int typeRet = rets[1];
            if (typeRet >= fallout::ROLL_CRITICAL_FAILURE && typeRet <= fallout::ROLL_CRITICAL_SUCCESS) {
                type = (typeRet > fallout::ROLL_FAILURE) ? 8 : 11; // returned new type (8 = SUCCESS, 11 = FAILURE)
            }
        }
    }
    EndHook();
...
static __declspec(naked) void wmWorldMap_hook()
{
    __asm {
		...
		jmp  fallout::mapLoadById; // eax - map id
    }
}
...
static __declspec(naked) void wmRndEncounterOccurred_hook()
{
    ...
    __asm {
		...
		test edx, edx;
		jz   hookRun;
		jmp  fallout::wmInterfaceRefresh;
hookRun: /////////////////////////////////
		push edx;
		push ecx;
		push ecx; // encType
...
break:
		...
		call fallout::wmMatchAreaContainingMapIdx;
		mov  eax, hkEncounterMapID;
noCar:
		call fallout::mapLoadById;
		xor  eax, eax;
...
static __declspec(naked) void roll_check_hook()
{
    __asm {
		...
		call fallout::randomTranslateRoll;
...
void Inject_UseSkillOnHook()
{
    ...
    SafeWriteBatch<DWORD>((DWORD)&sourceSkillOn, {
                                                     0x4AAF47, 0x4AB051, 0x4AB3FB, 0x4AB550, 0x4AB8FA, 0x4ABA54, 0x4AB0EF, 0x4AB5C0, 0x4ABAF2 // fix for time increment
                                                 });
}
