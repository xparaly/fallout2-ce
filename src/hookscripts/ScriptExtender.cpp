...
void InitScriptProgram(fallout::ScriptProgram &prog, const char* fileName, bool fullPath) {
	...
	if (scriptPtr) {
		...
		for (int i = 0; i < SCRIPT_PROC_COUNT; ++i) {
...
void RunScriptProc(fallout::ScriptProgram* prog, long procId) {
    if (procId > 0 && procId < SCRIPT_PROC_COUNT) {
...
{
    int procPosition = scr->procs[SCRIPT_PROC_COUNT];
	if (procPosition != -1) {
		fallout::_executeProcedure(scr->program, procPosition);
	}
	return procPosition;
}
