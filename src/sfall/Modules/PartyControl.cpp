...
static struct DudeState {
    fallout::Object* obj_dude = nullptr;
...
struct PartySneakWorking {
    fallout::Object* object;
    DWORD sneak_queue_time;
    DWORD sneak_working;
};
static std::vector<PartySneakWorking> partySneakWorking;

static void __fastcall RemoveSneakWorking(fallout::Object* object)
{
...
static bool SetAddictGvar(fallout::Object* npc)
{
    bool isAddict = false;
...
static void SetCurrentDude(fallout::Object* npc)
{
...
int __fastcall PartyControl::SwitchHandHook(fallout::Object* item)
{
    ...
    //	}
    // }
    // return -1 // this is the original
    return 1; // this the lazy workaround
}

static long __fastcall GetRealDudePerk(fallout::Object* source, long perk)
{
    // if (isControllingNPC && source == realDude.obj_dude) {
    //	return realDude.perkLevelDataList[perk];
    // }
    // return fo::func::perk_level(source, perk);
    return 1; // this is lazy workaround
}

static long __fastcall GetRealDudeTrait(fallout::Object* source, long trait)
{
    // if (isControllingNPC && source == realDude.obj_dude) {
    //	return (trait == realDude.traits[0] || trait == realDude.traits[1]) ? 1 : 0;
    // }
    // return fo::func::trait_level(trait); // this is original
    return 1; // this is lazy workaround
}
...
static void NPCWeaponTweak()
{
    ...
    //	fallout::Object* lItem = fo::func::inven_left_hand(fo::var::obj_dude);
    //	if (lItem) {
    //		isSwap = true;
    //		fallout::Object* rItem = fo::func::inven_right_hand(fo::var::obj_dude);
...
void PartyControl::SwitchToCritter(fallout::Object* critter)
{
...
fallout::Object* PartyControl::RealDudeObject()
{
...
static void __fastcall PartyMemberPrintStat(BYTE* surface, DWORD toWidth)
{
    ...
    // fallout::Object* partyMember = fo::var::dialog_target;
    // int xPos = 350;
...
struct PartyTarget {
    fallout::Object* member;
    fallout::Object* target;
};

static std::vector<PartyTarget> partyOrderAttackTarget;

static fallout::Object* GetMemberTarget(fallout::Object* member)
{
    // for (auto &el : partyOrderAttackTarget) {
    //	if (el.member->id == member->id) {
    //		fallout::Object* target = el.target;
...
static void SetMemberTarget(fallout::Object* member, fallout::Object* target)
{
    for (auto& el : partyOrderAttackTarget) {
        if (el.member->id == member->id) {
            el.target = target;
...
static void __fastcall action_attack_to(long unused, fallout::Object* partyMember)
{
    // fo::func::gmouse_set_cursor(20);
    // fo::func::gmouse_3d_set_mode(2);

    // fallout::Object* targetObject = nullptr;
    // fallout::Object* validTarget = nullptr;
    ...
    // do {
    //	fallout::Object* underObject = fo::func::object_under_mouse(1, 0, fo::var::map_elevation);
...
static void __fastcall SetOrderTarget(fallout::Object* attacker)
{
    // if (attacker->critter.teamNum || partyOrderAttackTarget.empty()) return;
    // fallout::Object* target = GetMemberTarget(attacker);
