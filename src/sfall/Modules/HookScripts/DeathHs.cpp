...
static DWORD __fastcall CalcDeathAnimHook_Script(DWORD damage, fallout::Object* target, fallout::Object* attacker, fallout::Object* weapon, int animation, int hitBack) {
	BeginHook();
	argCount = 5; // was 4
	...
	if (registerHookDeathAnim1) {
		...
		if (cRet > 0) {
			...
			if (objectCreateWithPid(&object, pid) != -1) { // create new object
				createNewObj = true;
	...
	DWORD animDeath = _pick_death(attacker, target, weapon, damage, animation, hitBack); // vanilla pick death
	...
	if (createNewObj) objectDestroy(weapon, 0); // delete created object
...
static __declspec(naked) void CalcDeathAnim2Hook() {
	__asm {
		call fallout::_check_death; // call original function
		mov  ebx, eax;
		call BeginHook;
		mov  eax, [esp + 60];
...
static __declspec(naked) void OnDeathHook2() {
	__asm {
		call fallout::partyMemberRemove;
		HookBegin;
		mov  args[0], esi;
		pushadc;
	}
